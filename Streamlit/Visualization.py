...
def calculate_sales_revenue(price, total_salse):
    revenue = price * total_salse
    return revenue
...
def main():
    ...
    if plot_type == "matplotlib" : # 해당 라이브러리 radio를 누르면 그 라이브러리 그래프로 바뀜
        plot_matplotlib()    
    ...
    st.write(col_name) # 종을 선택하는 selectbox를 만듬

    result = iris.loc[iris['species'] == col_name].reset_index(drop=True)
    st.dataframe(result) # 종을 선택하면 결과 값이 바뀜

    fig, ax = plt.subplots()
    sns.scatterplot(result, x = 'sepal_length', y = 'sepal_width', ax=ax)
    st.pyplot(fig) #종을 선택할 때마다 그래프가 바뀜

    # MultiSelect 
    cols = st.multiselect("복수의 컬럼 선택", iris.columns) # 일반 selectbox와 리턴 값이 다름
    st.write(cols)
