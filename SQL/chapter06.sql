...
-- 쿼리의 순서를 기억한다!
-- SELECT부터 시작을 하는 것이 아닌 FROM -> WHERE -> GROUP BY -> HAVING -> SELECT -> ORDER BY 순
...
-- p.169
-- 상품별로 재구매율을 계산한 뒤,
-- 재구매율을 기준으로 랭크를 계산함(서브쿼리, Windows Function)
...
-- p.188
-- 각 분위수별 재구매 소요 기간의 분산을 구하려면 다음과 같은 정보를 결합해 구해야 함
-- 상품별 분위수 테이블
-- 주문 소요 시간 : orders 테이블
...
-- median 함수를 이용하겠다! (문제는 MySQL은 제공하지 않음)
-- 1) SQL에 함수가 존재하지 않음 ===> 사용자 정의 함수 (PL/SQL)
-- 2) Python 또는 R 이랑 연동을 해서, DB에서 Python으로 데이터를 가져와서 통계 처리
...
-- 사후분석 ==> 두 개 그룹으로 쪼개서, 두 그룹간의 평균 비교
